...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not DEEPSEEK_API_KEY:
            await update.message.reply_text("Ошибка: не настроен ключ DeepSeek API. Добавь переменную DEEPSEEK_API_KEY в Railway.")
            return
        ...
        logger.info(f"Отправка запроса к DeepSeek: {user_message}")
        ...
        if response.status_code == 200:
            ...
            logger.info("Успешно получили ответ от DeepSeek")
        else:
            bot_reply = f"Ошибка API: {response.status_code} - {response.text}"
        ...
        await update.message.reply_text(f"Произошла ошибка: {str(e)}. Попробуй ещё раз позже.")
...
def main():
    ...
    if not DEEPSEEK_API_KEY:
        logger.error("DEEPSEEK_API_KEY не найден! Добавьте переменную в Railway.")
        return
